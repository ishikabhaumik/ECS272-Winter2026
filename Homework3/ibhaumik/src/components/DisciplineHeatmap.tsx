...
export default function DisciplineHeatmap({
  ...
  useEffect(() => {
    ...
    svg
      ...
      .style('fill', '#444')
      .text('Click a discipline to filter the timeline. Click again to clear.');
    ...
    const cellRects = svg
      ...
      .attr('stroke', (d) => (d.discipline === selectedDiscipline ? '#111' : 'transparent'))
      .attr('stroke-width', (d) => (d.discipline === selectedDiscipline ? 2 : 0))
      .style('cursor', 'pointer')
      .on('click', (_, d) => onSelectDiscipline(d.discipline === selectedDiscipline ? null : d.discipline));
    ...
    const tooltip = chartRef.current?.ownerDocument?.getElementById('heatmap-tooltip');
    if (tooltip) {
      cellRects
        .on('mouseenter', function (event, d) {
          d3.select(this).raise();
          const medalLabel = (d.medalType as string).replace(' Medal', '');
          tooltip.textContent = `${d.discipline} — ${medalLabel}: ${d.count}`;
  ...
  }, [data, size, selectedCountryCode, selectedCountryName, selectedDiscipline, onSelectDiscipline]);

  return (
    <div ref={chartRef} className="chart-container chart-container-heatmap">
      <div id="heatmap-tooltip" className="chart-tooltip" aria-hidden="true" />
