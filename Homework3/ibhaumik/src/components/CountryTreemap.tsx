...
export default function CountryTreemap({
  ...
  useEffect(() => {
    ...
    const root = d3
      .hierarchy({ children: data } as unknown as d3.HierarchyNode<MedalTotalRow>)
      .sum((d: unknown) => (d as MedalTotalRow).total ?? 0)
    ...
    type LeafNode = d3.HierarchyRectangularNode<MedalTotalRow>;
    const leavesData = (root as unknown as { leaves: () => LeafNode[] }).leaves();

    const chart = svg.append('g').attr('transform', `translate(${margin.left},${margin.top})`);

    const leaves = chart
      .selectAll<SVGGElement, LeafNode>('g')
      .data(leavesData)
      .join('g')
    ...
    const rects = leaves
      ...
      .attr('stroke', '#fff')
      .attr('stroke-width', 1)
      .attr('opacity', (d) => (d.data.country_code === selectedCountryCode ? 1 : 0.5))
    ...
    rects
      ...
      .attr('stroke-width', (d) => (d.data.country_code === selectedCountryCode ? 2.5 : 1))
      .attr('opacity', (d) => (d.data.country_code === selectedCountryCode ? 1 : 0.5));

    const tooltip = chartRef.current?.ownerDocument?.getElementById('treemap-tooltip');
    if (tooltip) {
      leaves
        .on('mouseenter', function (event, d) {
          const r = d3.select(this).select('rect');
          r.raise();
          r.transition().duration(150).attr('opacity', 0.9);
          tooltip.textContent = `${d.data.country}: Gold ${d.data.gold}, Silver ${d.data.silver}, Bronze ${d.data.bronze} — Total ${d.data.total}`;
        ...
        .on('mouseleave', function () {
          const r = d3.select(this).select('rect');
          const leafD = d3.select(this).datum() as d3.HierarchyRectangularNode<MedalTotalRow>;
          r.transition()
            .duration(200)
            .ease(d3.easeCubicInOut)
            .attr('opacity', leafD.data.country_code === selectedCountryCode ? 1 : 0.5);
  ...
  return (
    <div ref={chartRef} className="chart-container chart-container-treemap">
      <div id="treemap-tooltip" className="chart-tooltip" aria-hidden="true" />
